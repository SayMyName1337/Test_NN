...
class ModelBuilder:
    ...
    # Определение классов потерь как статические атрибуты класса
    class FocalLoss(tf.keras.losses.Loss):
        """Реализация Focal Loss для бинарной классификации"""
        
        def __init__(self, gamma=2.0, alpha=0.25, **kwargs):
            super(ModelBuilder.FocalLoss, self).__init__(**kwargs)
            self.gamma = gamma
            self.alpha = alpha
        ...
        def get_config(self):
            config = super(ModelBuilder.FocalLoss, self).get_config()
            config.update({
                'gamma': self.gamma,
                'alpha': self.alpha
            })
    ...
    class CategoricalFocalLoss(tf.keras.losses.Loss):
        ...
        def __init__(self, gamma=2.0, **kwargs):
            super(ModelBuilder.CategoricalFocalLoss, self).__init__(**kwargs)
            self.gamma = gamma
        ...
        def get_config(self):
            config = super(ModelBuilder.CategoricalFocalLoss, self).get_config()
            config.update({
                'gamma': self.gamma
            })
            return config
    ...
    def build_main_neural_network(self, input_shape, num_classes, hyperparams=None):
        ...
        if num_classes == 2:
            ...
            if loss_type == 'default':
                ...
                # Используем класс для Focal Loss
                gamma = hyperparams.get('focal_gamma', 2.0)
                loss = self.CategoricalFocalLoss(gamma=gamma)
            else:
                loss = 'categorical_crossentropy'
...
class ExperimentRunner:
    ...
    def run_experiment(self, noise_type, noise_range, noise_step, n_experiments=3, use_preprocessing=True):
        ...
        nn_params = self.model_builder.optimize_neural_network(
            X_train_selected, y_train, X_val_selected, y_val, input_shape, num_classes, n_trials=15, noise_type=noise_type # n_trials - попытки оптимизации
        )
