...
class ModelBuilder:
    ...
    class ImprovedAdaptiveEnsemble:
        ...
        def _calculate_model_weights(self, X, y):
            ...
            # Назначаем веса с учетом типа шума
            if noise_type == 'impulse':
                # KNN и Random Forest часто лучше с импульсным шумом
                weights['knn'] *= 1.5
                weights['random_forest'] *= 1.5
            elif noise_type == 'missing':
                # XGBoost и LightGBM часто лучше с пропущенными данными
                weights['xgboost'] *= 1.5
                weights['lightgbm'] *= 1.5
...
class ExperimentRunner:
    ...
    def run_experiment(self, noise_type, noise_range, noise_step, n_experiments=3, use_preprocessing=True):
        ...
        ensemble = self.model_builder.ImprovedAdaptiveEnsemble(models, X_val_selected, y_val, confidence_threshold=0.55) # Уверенность нейронной сети
